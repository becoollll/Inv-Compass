...
<body>
    ...
    <form id="form" name="form" method="post" class="center" action="focus.php">
        ...
        <div class="center">
            ...
            if (isset($_POST["Submit"])) {
                ...
                //若有新關注則新增reminder資料表
                //ETF ETN '存託憑證'
                if(isset($_POST['Submit']) && $cc != null && $no_company == 1){
    ...
    <div class="center" style="margin-top:3%;">
        ...
        <input class="display" type="button" value="顯示所有內容" onclick="javascript:window.location.href='focus2.php'">
        </p> 
    </div>
    <div>
        <?php
    ...
    <!--警示彈窗-->
    <style>
         .ui-widget-header, ui-buttons{
            background: #f27360; /*title框內*/
            border: 1px solid #9aaaba; /*title框*/
            color: #eeeeed;  /*title字顏色*/
            font-weight: bold; 
         }
        ...
        if($reminder == 0 ){ 
             ?>
              <script type="text/javascript">
		       ...
		       position: ['middle', 300],  //視窗位置
                       modal: true, //彈窗時背景變灰 凸顯彈窗
                       resize: function() {
      ...
      <div id="dialog" title="Reminder" class="content">
      <?php	
         $user = 'username';
         $pass = 'password';
         $db = 'database';
         $con = mysqli_connect("localhost", $user, $pass, $db);
		 $teamid = $_COOKIE['teamid']; 
		 $sql = "SELECT * FROM `reminder`,`company` WHERE `teamid` = '{$teamid}' and `flag3` = '1' and `reminder`.`companyid` = `company`.`cid`";
		 ...
		 while($data = mysqli_fetch_array($result,MYSQLI_ASSOC)){
		    if($data['flag2'] == 1){
		       if($com_alert != $data['companyid'])
		       {
		       	  ...
		       	  echo "<br>";
		       	  $com_alert = $data['companyid'];
		       	  echo $com_alert," ",$data['cname']," : ";
		       	  $temp = 1;
		       }
		       else
		       {
